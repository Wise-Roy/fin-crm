...
import { Search, Plus, Building2, X, Users, FolderPlus, IndianRupee, Trash2, Edit3 } from "lucide-react";
...
export function ClientsView({
  ...
  const filtered = clients.filter(
    (c) =>
      ...
      (c.client_group || []).some((g) => g.group_name.toLowerCase().includes(search.toLowerCase())),
  );
      ...
      <div className={`bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden flex flex-col transition-all ${selectedClient ? "w-full lg:w-[55%]" : "w-full"}`}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50 shrink-0">
          <h3 className="text-sm font-semibold text-gray-900">
            Clients <span className="text-gray-300 font-mono ml-1">{clients.length}</span>
          </h3>
          <div className="flex items-center gap-2">
            <div className="relative">
              <Search size={11} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-7 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900/10 w-32" />
            </div>
            {canAdd && (
              ...
              <div className="mx-5 mt-4 mb-2 p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {(([["Name *", "name", false], ["Email", "email", false], ["Phone", "phone", true]] as const)).map(([ph, key, mono]) => (
                    <input key={key} placeholder={ph} value={clientForm[key]} onChange={(e) => setClientForm((p) => ({ ...p, [key]: e.target.value }))} className={`border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-gray-900/10 bg-white ${mono ? "font-mono" : ""}`} />
                  ))}
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={submitClient} className="text-xs bg-gray-900 text-white px-3 py-1.5 rounded-lg hover:bg-gray-800 transition-colors">Add Client</button>
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden shrink-0">
              <div className="mx-5 mt-4 mb-2 p-4 bg-gray-50 rounded-xl border border-blue-100 space-y-3">
                ...
                <div className="space-y-2.5">
                  ...
                  <input type="tel" placeholder="Group Phone" value={groupPhone} onChange={(e) => setGroupPhone(e.target.value)} className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-gray-900/10 font-mono" />
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setAddMode(null)} className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-colors">Cancel</button>
                  <button onClick={submitGroup} disabled={!groupClientId || !groupName.trim()} className="text-xs bg-gray-900 text-white px-3 py-1.5 rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50">Add Group</button>
                </div>
              </div>
            </motion.div>
          )}
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!c.is_active && (<span className="text-[10px] bg-red-50 text-red-500 px-1.5 py-0.5 rounded font-mono">Inactive</span>)}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] text-gray-400 font-mono">{c.email || "No email"}</span>
                        {groupCount > 0 && (<span className="text-[10px] text-gray-400 flex items-center gap-0.5"><Users size={9} /> {groupCount} group{groupCount > 1 ? "s" : ""}</span>)}
                      </div>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-[11px] text-gray-400">{completedTasks}/{totalTasks} tasks</div>
                    {rev > 0 && userRole === "OWNER" && (
                      <div className="text-[10px] font-mono text-emerald-600">{fmtINR(rev)}</div>
                    )}
                  </div>
                </div>
              </motion.button>
          ...
          >
            <div className="flex items-start justify-between px-5 py-4 border-b border-gray-50 shrink-0">
              <div className="min-w-0 pr-4">
                <h3 className="text-sm font-semibold text-gray-900 truncate">{selectedClient.name}</h3>
            ...
            <div className="flex-1 overflow-auto p-5 space-y-5">
              ...
              {can(userRole, "view_revenue") && revenue && (
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                  <h4 className="text-[11px] font-semibold text-emerald-700 uppercase tracking-wider flex items-center gap-1 mb-3">
                    <IndianRupee size={10} /> Revenue
                  </h4>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <div className="text-lg font-mono font-semibold text-emerald-800">{fmtINR(revenue.total_paid)}</div>
                      <div className="text-[11px] text-emerald-600">{revenue.paid_count} paid payments</div>
                    </div>
                    <div>
                      <div className="text-lg font-mono font-semibold text-amber-700">{fmtINR(revenue.total_pending)}</div>
                      <div className="text-[11px] text-amber-600">{revenue.pending_count} pending</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Groups ({groups.length})</h4>
                  {canAdd && (
                    <button onClick={() => setShowDetailAddGroup(!showDetailAddGroup)} className="text-[11px] text-gray-500 hover:text-gray-900 flex items-center gap-1 transition-colors">
                    ...
                    groups.map((g) => {
                      ...
                      return (
                        <div key={g.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2.5 group/grp">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-gray-400 font-mono">{groupTasks.length} tasks</span>
                            {canAdd && !isEditing && (
              ...
              {/* Tasks */}
              <div>
                <h4 className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-3">Tasks ({clientTasks.length})</h4>
