...
import type { TaskStatus, Priority, ReimbStatus, PaymentStatus, Role, View, DscStatus } from "./types";
...
export const getDscStatus = (validTillDate: string): DscStatus => {
  const now = new Date();
  const validTill = new Date(validTillDate);
  if (validTill <= now) return "EXPIRED";
  const thirtyDaysFromNow = new Date();
  thirtyDaysFromNow.setDate(thirtyDaysFromNow.getDate() + 30);
  if (validTill <= thirtyDaysFromNow) return "EXPIRING_SOON";
  return "ACTIVE";
};

export const DSC_STATUS_CFG: Record<DscStatus, { label: string; cls: string }> = {
